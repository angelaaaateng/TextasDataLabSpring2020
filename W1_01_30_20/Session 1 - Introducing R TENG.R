...
setwd("/Users/angelateng/NYU_GoogleDrive/NYU ACADEMICS/Spring 2020/Text as Data/TextasDataLabSpring2020/W1_01_30_20")  # set working directory
...
# 2 WORKING WITH DATA
#-----------------------------
# COMMAND + SHIFT + C = COMMENT OR UNCOMMENT
...
# using base R to subset datatframes
# That was pretty impossible to read in the console, let's try this:
View(polling_data[, c("Pollster", "Number.of.Observations")]) # printing in the console but doesn't save output --> filtered df 
...
# tidyverse library = package is dplyr --> instead of base R to filter dataframes 
# Using pipe notation
polling_data %>% select(Pollster) %>% head(.,10) #take head with 10 observations 
polling_data %>% select(Pollster, Number.of.Observations) %>% head() 
# either us a . or use head. when you're doing a pipe, order matters 
# vs just wrapping your function
