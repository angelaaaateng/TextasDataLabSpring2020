...
# TOKEN TYPE RATTIO = TOTAL TYPES / TOTAL TOKENS 
...
num_types <- ntype(tokens)
num_types

nixon_inaug_df <- data.frame("num_tokens" = num_tokens, 
                              "num_types" = num_types,
                              "year" = data_corpus_inaugural$document$Year, 
                              "president" =data_corpus_inaugural$document$President)
nixon_inaug_df 

nixon_inaug_df  <- nixon_inaug_df %>% mutate(TTR = num_types / num_tokens)
...
TTR_by_year_president <- nixon_inaug_df %>% group_by(year, president) %>% summarise(mean_ttr = mean(TTR, na.rm = TRUE))
  
View(TTR_by_year_president)

# select only nixon 
nixon_ttr <- TTR_by_year_president[which (TTR_by_year_president$president=='Nixon'), ]
nixon_ttr 
